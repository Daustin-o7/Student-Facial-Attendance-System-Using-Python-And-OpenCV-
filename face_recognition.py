    ...
    def face_recog(self):
        def draw_boundary(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in features:
                ...
                #my_cursor.execute("SELECT std_id FROM student")
                #id = my_cursor.fetchone()
                #id=self.var_std_id.get()
                #id_str =str(id)
                
                my_cursor.execute("SELECT std_name FROM student WHERE std_id="+str(id))
                n = my_cursor.fetchone()  # Assuming fetchone() returns a tuple or None
                n = str(n[0]) if n else "Unknown"
                
                
                #if n is not None:
                #n = "+".join(n)  # Convert the tuple to a string
                #else:
                #    n = "Unknown"
                ...
                #if r is not None:
                #r = "+".join(r)  # Convert the tuple to a string
                #else:
                 #   r = "Unknown"
                ...
                #if d is not None:
                #d = "+".join(d)  # Convert the tuple to a string
                #else:
                 #   d = "Unknown"
                ...
                my_cursor.execute("SELECT year from student where std_id="+str(id))
                i = my_cursor.fetchone()
                i = i[0] if i else "Unknown"


                #if i is not None:
                #i = "+".join(i)  # Convert the tuple to a string
                #else:
                 #   i = "Unknown"
