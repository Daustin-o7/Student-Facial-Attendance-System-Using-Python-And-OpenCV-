...
class signup_window:
    ...
    def signup(self):
        if self.txtuser.get() == "" or self.txtpass.get() == "" or self.sectxt.get() == "" or self.conftxt.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host="localhost", user="root", password="D@1234567", database="face_recognizer")
                cursor = conn.cursor()
