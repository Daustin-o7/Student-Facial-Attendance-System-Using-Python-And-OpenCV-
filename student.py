...
class student:
    def __init__(self, root):
        ...
        dep_combo['values'] = ("Select Department", "AIML", "Cloud Computing", "Big Data",
                               "Cyber Security", "Computer", "IT", "Mechanical", "Chemistry", "Pharmacy")
        dep_combo.current(0)
        ...
        div_combo = ttk.Combobox(class_student_frame, textvariable=self.var_division, font=(
...
#------------------------------------------------- function declarartion ----------------------------------------------------------------
    ...
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get()=="" :
            ...
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="D@1234567", database="face_recognizer")
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(
            host="localhost", username="root", password="D@1234567", database="face_recognizer")
        my_cursor = conn.cursor()
    ...
    def get_cursor(self, event=""):
        ...
        # Check if data list contains the expected number of elements
        if len(data) >= 15:  # Adjust the number based on your actual data structure
            # Populate tkinter variables with data
            ...
            print("Error: Insufficient data in the selected row.")
        ...
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                ...
                if Update>0:
                    conn = mysql.connector.connect(host="localhost", username="root", password="D@1234567", database="face_recognizer")
                    my_cursor = conn.cursor()
                    my_cursor.execute("UPDATE student SET Dep=%s,course=%s,year=%s,semester=%s,std_name=%s,division=%s,roll=%s,gender=%s,dob=%s,email=%s,phone=%s,address=%s,teacher=%s,radio1=%s WHERE std_id=%s",(
                    self.var_dep.get(),
                    self.var_course.get(),
                    self.var_year.get(),
                    self.var_semester.get(),
